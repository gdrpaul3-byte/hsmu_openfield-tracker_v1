...
def process_stream(
    ...
    path_pts: List[Point] = []
    path_ts: List[float] = []
    # rollback: no segment validity filtering
    ...
    while True:
        ...
        # Stop when reaching the requested max duration (relative to start_frame)
        if max_duration_s is not None and max_duration_s > 0:
            elapsed_s = ts_s - (start_frame / fps)
            if elapsed_s >= float(max_duration_s):
                break
        ...
        if found:
            ...
            path_ts.append(ts_s)
            # rollback: no distance accumulation or validity filtering
            if len(path_pts) >= 2:
                pass
    # rollback: no lost-counter reset logic
            ...
            # rollback: no zone distance accumulation
    ...
    if zones is not None or stats:
        try:
            ...
            with open(out_path, "w", newline="", encoding="utf-8-sig") as f:
                w = csv.writer(f)
                w.writerow(["zone", "total_time_s", "visits", "mean_dwell_s", "max_dwell_s"])
                for name, data in stats.items():
                    ...
                    w.writerow([name, f"{total_s:.3f}", int(data["visits"]), f"{mean_dwell:.3f}", f"{max_dwell:.3f}"])
        except Exception:
            pass

    # export overall summary
    # rollback: no overall distance/velocity summary export
        ...
        try:
            ...
            # rollback: no zone distance/speed bar plots
        except Exception:
            pass
    ...
    try:
        if first_bg is not None and len(path_pts) > 1:
            ...
            # rollback: no distance/speed time-series plots
    except Exception:
        pass
...
def main():
    ...
    # Ask maximum tracking duration (seconds) before starting
    try:
        raw = input("최대 측정 시간(초, 기본 300): ").strip()
        max_sec = int(raw) if raw else 300
    except Exception:
        max_sec = 300
